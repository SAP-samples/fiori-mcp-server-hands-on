## Open Figma and login to it

1. Open [Figma](https://www.figma.com/login) in a browser, which will ask you to login.

2. Open the [Login File](../../figmaLogin.txt) and pick the login data for your assigned number. Enter the data in the Figma browser window to complete your login.

## Execute the following steps to open your design

1. Download the [Figma file](./figma_file/CA261%20Create%20great%20UX%20with%20AI,%20SAP%20Design%20System,%20SAP%20Fiori%20Elements%20and%20SAP%20UI5.fig) to your local machine.

   ![Figma](./images/exc1-0-1.png)

2. Open the file in the Figma web browser window.
* Click on **_Import_** in the top right corner.

   ![Figma](./images/exc1-0-2.png)

* In the dialog that appears, click on **_Import from computer_** and select the downloaded file.

   ![Figma](./images/exc1-0-3.png)

* Once the import is complete, click on **_Done_**.

   ![Figma](./images/exc1-0-4.png)

* Now, your **_Drafts_** should contain the imported file. Double-click on it.

   ![Figma](./images/exc1-0-5.png)

4. Explore the Figma interface
* The design will open in the current browser window.

   ![Figma](./images/exc1-0-6.png)

* In the left side panel, you’ll see the layer structure of the design.

   ![Figma](./images/exc1-0-7.png)

* In the right side panel, you’ll find settings and properties for each selected element in the main design area.

   ![Figma](./images/exc1-0-8.png)

Continue to - [Exercise 1.1 - Adjust the buttons in the object page header](../ex1.1/README.md)
